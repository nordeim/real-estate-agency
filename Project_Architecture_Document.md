...
| --- | --- | --- | --- |
| 1.8 | 2026-09-21 | Dependency & build hardening: 50 unused scaffold dependencies pruned (26 unused `@radix-ui/*` packages — only `react-select` is imported — plus dnd-kit, `@mdxeditor/editor`, `@tanstack/*`, recharts, zustand, vaul, cmdk, uuid, `z-ai-web-dev-sdk`, v3-era `tailwindcss-animate`, `sharp` (no `next/image` usage), …) — package.json now carries exactly the 15 runtime + 14 dev packages the app and its gates use; `next.config.ts` `typescript.ignoreBuildErrors` flipped to `false` (type-checking enforced at the build boundary — verified by the e2e webServer's production build); `describeDatabaseTarget()` added to `src/lib/db-path.ts` (6 tests, total 77 vitest) and wired into `scripts/with-db.ts` so an env-injected absolute `DATABASE_URL` outside the repo prints a stderr warning instead of silently redirecting `db:push`/`db:seed`; parity re-audit vs the live original (zero drift — mobile menu overlay verified byte-identical); screenshots refreshed | [CA] |
...
| Integration (infra) | `src/lib/db-path.test.ts` | 18 | Repo-root SQLite resolution contract: schema-anchored relative `file:` URLs, absolute/Postgres passthrough, missing-env passthrough, nested-CWD root discovery; `describeDatabaseTarget()` outside-repo verdict (env-injected absolute `file:` URLs produce the wrapper's warning; repo-root/relative/Postgres/no-repo cases stay silent) |
